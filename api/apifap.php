...
// jsonエンコードして必要なエスケープを行う
function jsonEncodeCharEscape($array){
  $enc = json_encode($array, JSON_UNESCAPED_UNICODE);
  $rtn = str_replace(["\r\n", "\r", "\n"], "\\n", $enc);
  return $rtn;
}
...
function sendPartOfSchedule(){
  ...
  $preSch = json_decode($preSch, true);
  // if ($preSch == NULL)  $preSch = [];
  $partOfSch = json_decode($partOfSch, true);
  // if ($partOfSch == NULL) $partOfSch = [];
  ...
  // echo('<br><br>');
  // echo ('final json<br>');
  $finalJson = json_encode($merged, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
  // var_dump($finalJson);
  ...
  $rt = unvEdit($mysqli, $sql);
  echo json_encode($rt, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
  if ($rt['result']){
    $mysqli->commit();
  }
  else{
...
function sendPartOfContact(){
  ...
  // var_dump('preCon');
  // var_dump($preCon);
  // var_dump('partOfContact');
  // var_dump($partOfContact);
  // if ($partOfContact == NULL) $partOfContact = [];
...
function fetchSomeState(){
  ...
  $rt = unvList($mysqli, $sql);
  for ($i = 0; $i < count($rt['dt']); $i++){
    $rt['dt'][$i]['state'] = json_decode($rt['dt'][$i]['state']);
  }
...
function fetchOfficeis(){
  ...
  $rt = unvList($mysqli, $sql);
  if (count($rt['dt'])){
    $rt['dt'][0]['others'] = json_decode($rt['dt'][0]['others']);
  }
  echo json_encode($rt, JSON_UNESCAPED_UNICODE);
  $mysqli->close();
}
...
function sendUsageFee(){
  ...
  $mysqli = connectDb();
  $sql = "
  insert into ahdusagefee (hid,bid,date,fee,hide)
  values ('$hid','$bid','$date','$fee','$hide')
  on duplicate key update
  fee = '$fee', hide = '$hide';
  ";
  $rt = unvEdit($mysqli, $sql);
...
function fetchUsageFee(){
  ...
  if ($bid){
    ...
    $sql = "
      select uf.date, uf.hid, uf.bid, uf.fee, uf.hide,
      com.hname, com.shname,
...
function nothing(){
  $rt['msg'] = 'function not found. hoge';
  $rt['request'] = $_REQUEST;
  $rt['post'] = $_POST;
  $rt['get'] = $_GET;
  echo json_encode($rt, JSON_UNESCAPED_UNICODE);
